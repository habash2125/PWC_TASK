...
NARROW = ScopePredicate(scope_key=SCOPE_KEY, values=(1, 2), unrestricted=False)
WIDE = ScopePredicate(scope_key=SCOPE_KEY, values=(), unrestricted=True)
NONE = ScopePredicate(scope_key=SCOPE_KEY, values=(), unrestricted=False)
...
async def test_missing_or_misplaced_predicates_are_repaired_never_widened(guard, allow, case_id, sql):
    ...
    assert result.sql_bound and "region_id IN (1, 2)" in result.sql_bound
...
async def test_allowed_corpus(guard, allow, case_id, sql):
    ...
    assert ":lens_scope" not in result.sql_bound
    if case_id not in {"no_scoped_view", "unscoped_reference_view"}:
        assert ":lens_scope" in result.sql_canonical
...
async def test_row_cap_is_always_present(guard, allow, settings):
    r = await guard.check(
        "SELECT order_id FROM v_orders WHERE region_id IN (:lens_scope_region_id)",
        allow=allow,
        scope=WIDE,
        mode="refresh",
    )
    ...
    r = await guard.check(
        "SELECT order_id FROM v_orders WHERE region_id IN (:lens_scope_region_id) LIMIT 999999",
        allow=allow,
        scope=WIDE,
        mode="refresh",
    )
    ...
    r = await guard.check(
        "SELECT order_id FROM v_orders WHERE region_id IN (:lens_scope_region_id) LIMIT 10",
        allow=allow,
        scope=WIDE,
        mode="refresh",
    )
...
async def test_scope_binding_per_executor(guard, allow):
    sql = "SELECT region_id, employee_name FROM v_employee_sales WHERE region_id IN (:lens_scope_region_id)"
    ...
    assert "IN (1, 2)" in narrow.sql_bound and "TRUE" in wide.sql_bound.upper()
...
async def test_string_scope_values_are_quoted_literals(allow):
    root = parse_one("SELECT category_name FROM v_products WHERE category_name IN (:lens_scope_category_name)")
    bound = bind_scope(root, ScopePredicate("category_name", ("Tools", "O'Brien")))
    assert "'Tools'" in bound and "O''Brien" in bound  # quotes escaped, never interpolated raw


async def test_unbound_placeholder_is_refused(allow):
    root = parse_one("SELECT category_name FROM v_products WHERE category_name IN (:lens_scope_other)")
    with pytest.raises(ValueError):
        bind_scope(root, NARROW)


async def test_repair_targets_the_reading_select_not_the_outer(allow):
    sql = "SELECT * FROM (SELECT region_id, SUM(revenue) AS s FROM v_monthly_sales GROUP BY region_id) t"
    repaired, n = repair_scope(parse_one(sql), allow.scope_columns)
    assert n == 1
    text = repaired.sql(dialect="sqlite")
    inner = sqlglot.parse_one(text, read="sqlite").find(sqlglot.exp.Subquery).this
    assert "lens_scope_region_id" in inner.args["where"].sql()
...
async def test_zero_queries_reach_executor_without_verified_scope(guard, allow, monkeypatch):
    ...
    corpus = [s for cid, s in ALLOWED + REPAIRED if "v_" in s and cid != "unscoped_reference_view"]
    for sql in corpus:
    ...
    for executed in seen:
        ...
        assert "region_id IN (1, 2)" in executed
